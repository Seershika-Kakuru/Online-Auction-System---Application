...
try{
    ...
    if(isMemberIDEmpty ||isNameEmpty || isPasswordEmpty || isEmailEmpty || isPhoneEmpty || isAddressEmpty){
		%><p style="font-size:16pt;"><b>Some field is empty!! please make sure all fields are filled: <a href='Register.jsp'> try again!</a></b></p><%
	}
	else{
    if (rs.next()) {
    	%><p style="font-size:16pt;"><b>memberID exists, please try another <a href='Register.jsp'>try again</a></b></p><%
    } else {
		...
		String insertEndUser = "INSERT INTO enduser(userID)"
						+ "VALUES (?)";
		//Create a Prepared SQL statement allowing you to introduce the parameters of the query
	    PreparedStatement ps1 = con.prepareStatement(insertEndUser);
	  ...
	  //Run the query against the DB
		...
		session.setAttribute("userID", memberID); // the username will be stored in the session
